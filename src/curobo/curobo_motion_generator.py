...
class CuroboMotionGenerator:
    ...
    def load_robot_config(self, robot_config_file_name: str = "ur5.yml"):
        ...
        try:
            ...
            self.num_gripper_joint = robot_config["kinematics"]["cspace"]["num_gripper_joint"]
    ...
    def init_motion_gen(self):
        ...
        self.motion_gen_config = MotionGenConfig.load_from_robot_config(
            ...
            interpolation_dt=self.user_config["motion_generation"][
                "interpolation_dt"
            ],
            # Position threshold for motion generation
            position_threshold=self.user_config["motion_generation"][
                "position_threshold"
            ],
            # Rotation_threshold for motion generation
            rotation_threshold=self.user_config["motion_generation"][
                "rotation_threshold"
            ],
            ...
            # Maximum trajectory time in seconds allowed for trajectory optimization.
            maximum_trajectory_time=self.user_config["motion_generation"][
                "maximum_trajectory_time"
            ],
            # Maximum time step in seconds allowed for trajectory optimization.
            maximum_trajectory_dt=self.user_config["motion_generation"][
    ...
    def wxyz_to_xyzw(self, quaternion: List[float]) -> List[float]:
    ...
    def fk(
        ...
        # Convert quaternion from [w, x, y, z] to [x, y, z, w]
        end_effector_orientation_xyzw = self.wxyz_to_xyzw(end_effector_orientation_wxyz)
    ...
    def motion_generate(
        ...
        link_pose_arrays: Optional[Dict[str, List[float]]] = None,
    ):
        ...
        Args:
            ...
            link_poses:
                goal other link poses in format of link name - [x, y, z, qx, qy, qz, qw]
        ...
        if goal_joint_angles is None and goal_end_effector_pose is not None:
            ...
            # Get link pose
            link_poses = None
            if link_pose_arrays is not None:
                link_poses = {}
                for link_name in link_pose_arrays.keys():
                    link_pose_array = link_pose_arrays[link_name]
                    link_pose = Pose(
                        position=self.tensor_args.to_device(
                            [link_pose_array[0], link_pose_array[1], link_pose_array[2]]
                        ),
                        # [qx,qy,qz,qw] to [qw,qx,qy,qz]
                        quaternion=self.tensor_args.to_device(
                            [link_pose_array[6], link_pose_array[3], link_pose_array[4], link_pose_array[5]]
                        ),
                        name=link_name,
                    )
                    link_poses[link_name] = link_pose
                    self.logger.log_debug(f"Pose Instance for link {link_name}:\n{link_pose}\n")

            # Plan single(cartesian space)
            try:
                self.logger.log_info("Generating Motion...")
                motion_gen_result = self.motion_gen.plan_single(
                    start_joint_angles, goal_pose, self.motion_gen_plan_config, link_poses
                )
